...
function sjcap(jfldcls) {
    ...
    var codeHTML = "<div class='input-group'>"
        + "<img src='" + decodeURIComponent(imgdir) + "/" + imgid + ".jpg' width='318' height='80' id='id' style='border:1px solid #ccc;'>"
        + "</div>"
        + "<br><div class='input-group'>"
        + "<input id='uword' name='uword' class='form-control' type='text' placeholder='Saisir Captcha'>"
    ...
    $('#captcha').html(codeHTML);

    $('#uword').on("change", function (e) {
        return jcap();
    });
}
...
function jcap() {
    ...
    if (uword === cword[anum - 1]) {
        ...
        $.smallBox({
            title: "",
            content: "Captcha incorrect ! Veuillez vérifier.",
            color: "#a90329",
            iconSmall: "fa fa-times-circle fa-2x bounce animated",
            timeout: 4000,
        });

        $('#uword').css('border-color', 'red');
