...
// http://perfectionkills.com/whitespace-deviations/
    ...
    var InputCellEditor = Backgrid.InputCellEditor = CellEditor.extend({
         ...
         */
        render: function () {
            var a = this.formatter.fromRaw(this.model.get(this.column.get("name")));
            if (a === undefined) {
                var pathValue = this.column.get("name");
                var attributes = pathValue.split('_');
                var instruction = 'this.$el.val(this.formatter.fromRaw(this.model.get("' + attributes[0] + '")';
                for (var i = 1; i < attributes.length; i++) {
                    instruction = instruction.concat('.' + attributes[i]);
                }
                eval(instruction + "))");
            }
            else {
        ...
        saveOrCancel: function (e) {
            ...
            if (e.type === "keydown") {
                ...
                if (e.keyCode === 13 || e.keyCode === 9) {
                    e.preventDefault();
                    //Hassen: cette condition if (a === undefined) est ajouter par moi pour gerer les Nested Models
                    if (a === undefined) {
                        var valueToSet = this.formatter.toRaw(this.$el.val());
                        var pathValue = this.column.get("name");
                        var attributes = pathValue.split('_');
                        var instruction = 'this.model';
                        var instruction2 = 'this.model';
                        var instructionAttr = ' this.model.attributes'
                        for (var i = 0; i < attributes.length; i++) {
                            if (i != attributes.length - 1) {
                                instruction = instruction.concat('.' + attributes[i]);
                                instruction2 = instruction2.concat('.' + attributes[i]);
                                instructionAttr = instructionAttr.concat('.' + attributes[i]);
                            }
                            else {
                                instruction = instruction.concat('.set("' + attributes[i] + '","' + valueToSet + '",{validate: true})');
                                instruction2 = instruction2.concat('.get("' + attributes[i] + '")');
                                instructionAttr = instructionAttr.concat('.' + attributes[i]);
                            }
                        }

                        if (_.isUndefined(valueToSet) || !eval(instruction)) {
                            this.trigger("error");
                        }
//                    
                        else {
                            ...
                            eval(instructionAttr + "=" + instruction2);
                            this.trigger("done");
                            // alert(this.$el.val());// Hassen:Pour recupérer le valeur du cellule a modifier
                        }
                    }
                    else {
                        var valueToSet = this.formatter.toRaw(this.$el.val());
...
//                    
                ...
                if (a === undefined) {
                    var pathValue = this.column.get("name");
                    var attributes = pathValue.split('_');
                    var instruction = 'this.formatter.fromRaw(this.model.get("' + attributes[0] + '")';
                    for (var i = 1; i < attributes.length; i++) {
                        instruction = instruction.concat('.' + attributes[i]);
                    }

                    if (eval(instruction + ")") === this.$el.val()) {
                        this.trigger("done");
                    }
                    else {
                        var self = this;
    ...
    var HeaderCell = Backgrid.HeaderCell = Backbone.View.extend({
        ...
        _resetCellDirection: function (sortByColName, direction, comparator, collection) {
            if (collection == this.collection) {
                if (sortByColName !== this.column.get("name"))
                    this.direction(null);
                else
    ...
    var Grid = Backgrid.Grid = Backbone.View.extend({
        ...
        initialize: function (options) {
            ...
            this.listenTo(this.columns, "reset", function () {
                ...
                if (this.footer)
                    this.footer = new (this.footer.remove().constructor)(options);
                this.render();
            });
        },
        /**
